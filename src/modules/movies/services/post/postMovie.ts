...
// Types
import type { CreateMoviePayload, CreateMovieResponse } from "../../types";

export default async function postRating(
  payload: CreateMoviePayload
): Promise<Either<ApplicationError, CreateMovieResponse>> {
  try {
    const { data: response } = await AxiosClient.post<CreateMovieResponse>(
      "movies",
      payload
    );
